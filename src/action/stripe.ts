...
// Get subscription price ID from environment variable with fallback to data.ts
const getSubscriptionPriceId = () => {
  const priceId = process.env.STRIPE_SUBSCRIPTION_PRICE_ID;
  if (!priceId) {
    console.warn("STRIPE_SUBSCRIPTION_PRICE_ID not set in environment variables");
    // Fallback to hardcoded value if env var not set
    return "price_1RKTQaIld5Bk5htqA7t1HWy4";
  }
  return priceId;
};
...
export const onGetStripeClientSecret = async (
  ...
  try {
    ...
    const subscription = await stripe.subscriptions.create({
      ...
      items: [{ price: getSubscriptionPriceId() }],
